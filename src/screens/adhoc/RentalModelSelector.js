...
@inject("adhocStore")
@observer
class RentalModelSelector extends Component {
    ...
    componentDidMount() {
        ...
        let rentalModels = this.props.route.params.hasOwnProperty("rentalModels")?this.props.route.params.rentalModels:[];
        this.setState({selectedItem,rentalModels});
        setTimeout(() => {
    ...
    render() {
        ...
        let RentalModelsArray = this.state.rentalModels.length > 0 ? this.state.rentalModels : this.props.adhocStore.RentalModels;
        let rentalModels = RentalModelsArray.filter(word =>
        ...
        return (
            <SafeAreaView style={{flex: 1, backgroundColor: colors.WHITE}}>
                <View style={styles.container}>
                    ...
                    />
                    <RenderPickerHeader name={"Rental Models"} onGoBack={this.onGoBack}/>
                            ...
                            return (
                                <React.Fragment>
                                    <TouchableDebounce
                                        ...
                                        onPress={() => {
                                            if(this.state.rentalModels.length>0){
                                                this.props.route.params.onRentalSelected(item);
                                                this.props.navigation.goBack();
                                            }else{
                                                this.props.adhocStore.setRentalModel(item.Name);
                                                this.props.navigation.goBack();
                                            }
                                        }}
                                    >
...
export default RentalModelSelector;

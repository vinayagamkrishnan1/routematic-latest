...
@inject("adhocStore")
@observer
class NonShiftTimeSelector extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({selectedItem,time});
        setTimeout(() => {
    ...
    render() {
        ...
        let timings = this.state.time.filter(word =>
            word.FromTime.toUpperCase().includes(this.state.text.toUpperCase())
        );
        return (
            <SafeAreaView style={{flex: 1, backgroundColor: colors.WHITE}}>
                <View style={styles.container}>
                    ...
                    />
                    <RenderPickerHeader name={"Shift Time "} onGoBack={this.onGoBack}/>
                    ...
                    <FlatList
                        ...
                        renderItem={({item, index}) => {
                            let isSelected = this.state.selectedItem ? this.state.selectedItem.toString().includes(item.FromTime) : false;
                            return (
                                <React.Fragment>
                                    <TouchableDebounce
                                        key={index}
                                        ...
                                        onPress={() => {
                                            this.props.adhocStore.handleTimePicked(item.FromTime);
                                            this.props.navigation.goBack();
                                        }}
                                    >
                                        <Text
...
export default NonShiftTimeSelector;

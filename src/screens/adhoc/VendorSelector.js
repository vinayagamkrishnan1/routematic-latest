...
class VendorSelector extends Component {
    ...
    componentDidMount() {
        ...
        let vendors = this.props.route.params.hasOwnProperty("vendors")?this.props.route.params.vendors:[];
        this.setState({selectedItem,vendors});
        setTimeout(() => {
    ...
    render() {
        ...
        let VendorsArray = this.state.vendors.length > 0 ? this.state.vendors : this.props.adhocStore.Vendors;
        let vendors = VendorsArray.filter(word =>
        ...
        return (
            <SafeAreaView style={{flex: 1, backgroundColor: colors.WHITE}}>
                <View style={styles.container}>
                    ...
                    />
                    <RenderPickerHeader name={"Vendors"} onGoBack={this.onGoBack}/>
                                    ...
                                    <TouchableDebounce
                                        ...
                                        onPress={() => {
                                            if(this.state.vendors.length>0){
                                                this.props.route.params.onVendorSelected(item);
                                                this.props.navigation.goBack();
                                            }else{
                                                this.props.adhocStore.setVendor(item.Name);
                                                this.props.navigation.goBack();
                                            }
                                        }}
                                    >

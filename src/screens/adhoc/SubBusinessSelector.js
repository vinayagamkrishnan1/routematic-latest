...
@inject("adhocStore")
@observer
class SubBusinessSelector extends Component {
    ...
    componentDidMount() {
        ...
        let subBusinesses = this.props.route.params.hasOwnProperty("subBusinesses")?this.props.route.params.subBusinesses:[];
        this.setState({selectedItem,subBusinesses});
        setTimeout(() => {
    ...
    render() {
        ...
        let SubBusinessesArray = this.state.subBusinesses.length > 0 ? this.state.subBusinesses : this.props.adhocStore.SubBusinesses;
        let subBusinesses = SubBusinessesArray.filter(word =>
        ...
        return (
            <SafeAreaView style={{flex: 1, backgroundColor: colors.WHITE}}>
                <View style={styles.container}>
                    ...
                    />
                    <RenderPickerHeader name={"Sub Businesses"} onGoBack={this.onGoBack}/>
                            ...
                            return (
                                <React.Fragment>
                                    <TouchableDebounce
                                        ...
                                        onPress={() => {
                                            if(this.state.subBusinesses.length>0){
                                                this.props.route.params.onSubBusinessSelected(item);
                                                this.props.navigation.goBack();
                                            }else{
                                                this.props.adhocStore.setSubBusiness(item.Name);
                                                this.props.navigation.goBack();
                                            }
                                        }}
                                    >
...
export default SubBusinessSelector;

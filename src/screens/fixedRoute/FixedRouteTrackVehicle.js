...
class FixedRouteTrackVehicleNEW extends Component {
    ...
    getWayPoints(shiftId, routeId) {
        console.warn('getWayPoints - ', shiftId, routeId);
        ...
        axios.get(url, {
            ...
            .then(respons => {
                console.warn('way point res - ', response);
    ...
    UNSAFE_componentWillMount() {
        ...
        const access_token = route.params.access_token ? route.params.access_token : "NA";
    ...
    shouldComponentUpdate(nextProps, nextState) {
        console.warn('on should comp update');
    ...
    render() {
        ...
        console.warn('this.state.trackWayPoints = ', this.state.trackWayPoints);
        let sourceLocation = {
            latitude: 0, longitude: 0,
            latitudeDelta: this.state.latitudeDelta,
            longitudeDelta: this.state.longitudeDelta
        }
        if(this.state.trackWayPoints?.length > 0){
            let data = this.state.trackWayPoints[0];
            console.warn('waypoint data - ', data);
            sourceLocation={
        ...
        return (
            <SafeAreaView>
                ...
                <View style={{backgroundColor: colors.WHITE, height: 100, margin: 10, padding: 5, borderRadius: 5, shadowColor: colors.GRAY, shadowRadius: 2, shadowOffset: 5}}>
                    {_renderFixedRouteDriverDetails(
                        Trip.DriverPhoto,
                        Trip.DriverName,
                        Trip.VehicleRegNo,
                        Trip.RouteNumber,
                        Trip.CallDriverEnabled,
                        this
                    )}
                </View>
                </View>
...
export default FixedRouteTrackVehicleNEW;
